...
function confirmKeywordSelection() {
  ...
  draft.keywordMatchingEnabled = draft.keywordRuleIds.length > 0
  change()
  keywordDialog.value = false
}
function toggleKeywordMatching(value) {
  if (!value) { setSwitch('keywordMatchingEnabled', false); return }
  if (!profile.value) { emit('warning', '请先选择关键字方案'); return }
  if (!draft.keywordRuleIds?.length) { openKeywordDialog(); return }
  setSwitch('keywordMatchingEnabled', true)
}
  ...
  <aside class="inspector-panel">
    ...
    <template v-else>
      ...
      <Form class="form-section" layout="vertical" size="small">
        ...
        <Button class="more-settings-button" size="small" @click="moreSettings = !moreSettings"><SettingOutlined />{{ moreSettings ? '收起更多设置' : '更多设置' }}</Button>
        <div v-if="moreSettings" class="field-grid serial-tuning-grid">
      ...
      <Form class="form-section" layout="vertical" size="small">
        <div class="policy-grid">
          ...
          <div class="policy-item"><strong>关键字匹配</strong><Switch class="capsule-switch" size="small" :checked="Boolean(draft.keywordMatchingEnabled)" @change="toggleKeywordMatching" /></div>
        </div>
        ...
        <Button type="primary" block :loading="busy" @click="save"><template #icon><SaveOutlined /></template>{{ uploadReady ? '保存上传/通道配置' : '保存通道配置' }}</Button>
      </Form>
      ...
      <div class="dashboard-section">
        <div class="section-title"><strong>关键字仪表盘</strong><Tooltip title="重新加载命中记录"><Button aria-label="重新加载命中记录" size="small" shape="circle" :disabled="!device.sessionId" @click="emit('reset-hits')"><template #icon><ReloadOutlined /></template></Button></Tooltip></div>
