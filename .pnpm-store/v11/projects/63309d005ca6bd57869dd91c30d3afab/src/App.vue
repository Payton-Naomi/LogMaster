...
const detectedPortNames = computed(() => new Set(ports.value.map((port) => String(port.name || '').toUpperCase())))
const isPortDetected = (portName) => Boolean(portName) && detectedPortNames.value.has(String(portName).toUpperCase())
const portStatusText = (portName) => !portName ? '未选择端口' : (isPortDetected(portName) ? portName : `${portName}（当前未检测到）`)
...
async function refreshAll() {
  try {
    ...
    if (!devices.value.some((device) => device.deviceId === selectedDevice.value)) {
...
function connectAll() {
  return withBusy(async () => {
    for (const device of devices.value.filter((item) => (item.status === 'disconnected' || item.status === 'error') && isPortDetected(item.config?.portName))) {
...
function startTask() {
  return withBusy(async () => {
    for (const device of devices.value.filter((item) => (item.status === 'disconnected' || item.status === 'error' || item.status === 'stopped') && isPortDetected(item.config?.portName))) await invoke('ConnectDevice', device.config)
...
onMounted(async () => {
  ...
  if (window.runtime?.EventsOn) {
    ...
    disposePorts = window.runtime.EventsOn('serial:ports', refreshAll)
  }
})
    ...
    <div class="content-shell">
      ...
      <aside class="device-pane">
        <div class="pane-heading">
          <div><h1>串口通道</h1><span>检测到 {{ devices.length }} 个</span></div>
          <button class="icon-button" title="扫描串口" :disabled="busy" @click="refreshAll"><RefreshCw :size="17" /></button>
        </div>
        <div class="batch-actions">
          <button class="primary" :disabled="busy || !devices.length" @click="connectAll"><Link :size="15" />全部连接</button>
          <button :disabled="busy || !devices.length" @click="disconnectAll"><Link2Off :size="15" />全部断开</button>
        </div>
        <div class="device-list">
          <button v-for="(device, deviceIndex) in devices" :key="device.deviceId" class="device-row" :class="{ selected: selected?.deviceId === device.deviceId }" @click="selectedDevice = device.deviceId">
            ...
            <span class="device-main"><strong>{{ device.name || device.deviceId }}</strong><small>{{ portStatusText(device.config?.portName) }} · {{ device.config?.baudRate || 115200 }}</small></span>
            <span class="status-dot" :class="statusClass(device.status)"></span>
            <span class="status-text">{{ statusLabel(device.status) }}</span>
          </button>
          <div v-if="!devices.length" class="empty-small">当前未检测到可用串口</div>
        </div>
      ...
      <section class="workspace">
        <div class="workspace-toolbar">
          <div class="device-title"><span class="status-dot" :class="statusClass(selected?.status)"></span><div><h2>{{ selected?.name || '选择设备' }}</h2><p>{{ portStatusText(selected?.config?.portName) }} · {{ statusLabel(selected?.status) }}</p></div></div>
          <div class="toolbar-actions">
            <button v-if="selected?.status === 'disconnected' || selected?.status === 'error'" class="primary" :disabled="busy || !selected || !isPortDetected(selected?.config?.portName)" @click="connect(selected)"><Link :size="16" />连接</button>
            <button v-else :disabled="busy || !selected" @click="disconnect(selected)"><Link2Off :size="16" />断开</button>
            <button class="success" :disabled="busy || !devices.length" @click="startTask"><Play :size="16" />开始采集</button>
            <button :disabled="busy || !devices.length" @click="stopTask"><CircleStop :size="16" />停止</button>
          </div>
        </div>
        ...
        <form class="command-bar" @submit.prevent="sendCommand"><span>&gt;</span><input v-model="command" :disabled="!selected?.deviceId" placeholder="向当前设备发送指令" /><button class="icon-button" type="submit" title="发送指令" :disabled="!selected?.deviceId || !command.trim()"><Send :size="17" /></button></form>
      </section>

      <aside class="inspector">
        <div class="inspector-section"><h3>通道配置</h3><label>设备名称<input v-model="selected.config.name" :disabled="!selected?.deviceId" /></label><label>串口<select v-model="selected.config.portName" disabled><option :value="selected.config.portName">{{ selected.config.portName || '未检测到串口' }}</option></select></label><div class="field-grid"><label>波特率<input v-model.number="selected.config.baudRate" type="number" min="300" max="4000000" :disabled="!selected?.deviceId" /></label><label>数据位<select v-model.number="selected.config.dataBits" :disabled="!selected?.deviceId"><option :value="5">5</option><option :value="6">6</option><option :value="7">7</option><option :value="8">8</option></select></label><label>停止位<select v-model.number="selected.config.stopBits" :disabled="!selected?.deviceId"><option :value="1">1</option><option :value="2">2</option></select></label><label>校验位<select v-model="selected.config.parity" :disabled="!selected?.deviceId"><option value="none">无</option><option value="odd">奇校验</option><option value="even">偶校验</option><option value="mark">标记</option><option value="space">空格</option></select></label></div><label>流控<select v-model="selected.config.handshake" :disabled="!selected?.deviceId"><option value="none">无</option><option value="rtscts" disabled>RTS/CTS（当前驱动不支持）</option><option value="xonxoff" disabled>XON/XOFF（当前驱动不支持）</option></select></label><button class="primary config-save" :disabled="busy || !selected?.deviceId" @click="saveDeviceConfig"><Settings :size="15" />保存通道配置</button></div>
      ...
      <div class="settings-band"><Settings :size="26" /><div><h2>Windows 当前检测到 {{ ports.length }} 个串口</h2><p>{{ ports.length ? ports.map((port) => `${port.name}${port.product ? ` · ${port.product}` : ''}`).join('；') : '当前没有检测到串口，连接操作已禁用。' }}</p></div></div>
      </main>
    </div>
