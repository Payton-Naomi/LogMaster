...
import { FolderOpenOutlined, ReloadOutlined, SaveOutlined, TeamOutlined } from '@ant-design/icons-vue'
const props = defineProps({ settings: { type: Object, default: () => ({}) }, invoke: Function })
const emit = defineEmits(['saved'])
const DEFAULT_SETTINGS = {
  defaultLogDirectory: 'D:\\LogMaster\\LocalLog', defaultSaveEnabled: true, defaultUploadEnabled: false,
  segmentMaxAgeSeconds: 1800, segmentMaxBytes: 128 * 1024 * 1024, noLogTimeoutSeconds: 1800,
  maxLogLines: 100000, logFontSize: 12, autoWrap: true, maxDiskBytes: 50 * 1024 * 1024 * 1024,
  storageWarningPercent: 80, autoDeleteUploaded: false, uploadedRetentionHours: 0, backendUrl: 'http://localhost:8080/api',
  uploadIntervalSeconds: 300, uploadConcurrency: 5, uploadGzip: true, programName: 'LogMaster采集端',
  programVersion: '0.0.10', buildVersion: '0.0.10', updateDate: '2026-08-14',
  companyName: '上海七十迈数字科技有限公司', communityTitle: '飞书交流群',
  communityText: '使用说明、问题反馈、获取最新版本请扫码加入飞书交流群。', communityUrl: '',
}
const draft = reactive({ ...DEFAULT_SETTINGS })
const state = reactive({ dirty: false, message: '' })
const storageUnit = ref('GB')
const clonePlain = (value) => JSON.parse(JSON.stringify(value || {}))
function applySettings(value) { Object.assign(draft, DEFAULT_SETTINGS, clonePlain(value)); storageUnit.value = Number(draft.maxDiskBytes || 0) >= 1024 ** 3 ? 'GB' : 'MB'; state.dirty = false }
watch(() => props.settings, applySettings, { immediate: true, deep: true })
onMounted(async () => { try { applySettings(await props.invoke('GetAppSettings')) } catch (_) {} })
...
async function save() { try { await props.invoke('SaveAppSettings', clonePlain(draft)); notification.success({ message: '设置已保存并生效', placement: 'bottomRight', duration: 3 }); state.dirty = false; emit('saved') } catch (_) {} }
...
async function reload() { try { applySettings(await props.invoke('ReloadAppSettings')); notification.success({ message: '配置文件已重新加载并生效', placement: 'bottomRight' }); emit('saved') } catch (_) {} }
</script>
<template>
  <main class="page-view">
    <div class="page-heading"><h1>全局设置</h1><div><Tooltip title="重新读取 settings-config.json"><Button @click="reload"><template #icon><ReloadOutlined /></template>重新加载</Button></Tooltip><Button type="primary" :disabled="!state.dirty" @click="save"><template #icon><SaveOutlined /></template>保存设置</Button></div></div>
    <div class="settings-grid">
      ...
      </Form><div class="setting-row"><strong>默认保存本地日志</strong><Switch class="capsule-switch" :checked="Boolean(draft.defaultSaveEnabled)" @change="(value) => { draft.defaultSaveEnabled = value; change() }" /></div><div class="setting-row"><Tooltip title="关闭后允许横向滚动"><strong>默认自动换行</strong></Tooltip><Switch class="capsule-switch" :checked="Boolean(draft.autoWrap)" @change="(value) => { draft.autoWrap = value; change() }" /></div></section>
      ...
      <section class="settings-card"><h2>云端上传</h2><Form layout="vertical"><Tooltip title="保存后立即用于后续请求"><FormItem label="后端地址"><Input v-model:value="draft.backendUrl" placeholder="http://host:port/api" @input="change" /></FormItem></Tooltip><div class="settings-fields"><FormItem label="上传间隔（秒）"><InputNumber v-model:value="draft.uploadIntervalSeconds" :min="1" @change="change" /></FormItem><FormItem label="并发数"><InputNumber v-model:value="draft.uploadConcurrency" :min="1" :max="16" @change="change" /></FormItem></div></Form><div class="setting-row"><Tooltip title="按服务端契约发送"><strong>GZIP 压缩</strong></Tooltip><Switch class="capsule-switch" :checked="Boolean(draft.uploadGzip)" @change="(value) => { draft.uploadGzip = value; change() }" /></div><div class="setting-row"><Tooltip title="上传失败原因与重试轨迹记录在本地日志文件"><strong>诊断日志</strong></Tooltip><Button size="small" @click="openLogs"><template #icon><FolderOpenOutlined /></template>打开日志目录</Button></div></section>
