...
async function resetHits() {
  ...
  try {
    const ids = selected.value.config?.keywordRuleIds || []
    const results = await Promise.all(ids.map((id) => props.desktop.invoke('GetKeywordHits', selected.value.sessionId, id)))
    ids.forEach((id, index) => { hitCache[id] = results[index] || [] })
  } catch (_) {}
}
