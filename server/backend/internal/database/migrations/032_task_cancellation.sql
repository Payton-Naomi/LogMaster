...
ALTER TABLE logmaster_api.log_uploads
    ADD CONSTRAINT log_uploads_status_check CHECK (status IN ('uploading', 'queued', 'parsing', 'completed', 'failed', 'cancelled'));
...
ALTER TABLE logmaster_api.parse_tasks
    ADD CONSTRAINT parse_tasks_status_check CHECK (status IN ('queued', 'running', 'completed', 'failed', 'cancelled'));
