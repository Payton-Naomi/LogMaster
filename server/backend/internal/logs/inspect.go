...
func (s *Service) inspectHandler(w http.ResponseWriter, r *http.Request) {
	...
	r.Body = http.MaxBytesReader(w, r.Body, capacity.MaxUploadBytes+(4<<20))
	...
	name := filepath.Base(strings.ReplaceAll(headers[0].Filename, "\\", "/"))
	path, _, err := saveUploadedFile(headers[0], name, root, capacity.MaxUploadBytes)
	if err != nil {
