...
import { Button, Descriptions, DescriptionsItem, Form, FormItem, Image as AntImage, Input, InputNumber, notification, Select, Switch } from 'ant-design-vue'
...
<template>
  <main class="page-view">
    <div class="page-heading"><div><h1>全局设置</h1><p>本地保存、日志窗口、存储保护和上传参数。</p></div><Button type="primary" :disabled="!state.dirty" @click="save"><template #icon><SaveOutlined /></template>保存设置</Button></div>
    <div class="settings-grid">
      ...
      </Form><div class="setting-row"><div><strong>默认自动换行</strong><small>关闭后允许横向滚动</small></div><Switch class="capsule-switch" :checked="Boolean(draft.autoWrap)" @change="(value) => { draft.autoWrap = value; change() }" /></div></section>
      <section class="settings-card"><h2>存储保护</h2><Form layout="vertical"><div class="settings-fields"><FormItem label="最大本地占用"><div class="unit-input"><InputNumber v-model:value="storageAmount" :min="1" @change="change" /><Select v-model:value="storageUnit" :options="[{ value: 'MB', label: 'MB' }, { value: 'GB', label: 'GB' }]" /></div></FormItem><FormItem label="预警阈值（%）"><InputNumber v-model:value="draft.storageWarningPercent" :min="1" :max="99" @change="change" /></FormItem></div></Form><div class="setting-row"><div><strong>自动删除已上传文件</strong><small>默认关闭；未上传和待核对文件永不自动删除</small></div><Switch class="capsule-switch" :checked="Boolean(draft.autoDeleteUploaded)" @change="(value) => { draft.autoDeleteUploaded = value; change() }" /></div></section>
      <section class="settings-card"><h2>云端上传</h2><Form layout="vertical"><FormItem label="后端地址"><Input v-model:value="draft.backendUrl" placeholder="http://host:port/api" @input="change" /><small class="inline-message" style="display:block;margin-top:4px">保存后重启程序生效；云端迁移时无需重装，改这里即可。</small></FormItem><div class="settings-fields"><FormItem label="上传间隔（秒）"><InputNumber v-model:value="draft.uploadIntervalSeconds" disabled /></FormItem><FormItem label="并发数"><InputNumber v-model:value="draft.uploadConcurrency" disabled /></FormItem></div></Form><div class="setting-row"><div><strong>GZIP 压缩</strong><small>按服务端契约发送</small></div><Switch class="capsule-switch" :checked="Boolean(draft.uploadGzip)" @change="(value) => { draft.uploadGzip = value; change() }" /></div><div class="setting-row"><div><strong>诊断日志</strong><small>上传失败原因与重试轨迹记录在本地日志文件</small></div><Button size="small" @click="openLogs"><template #icon><FolderOpenOutlined /></template>打开日志目录</Button></div></section>
      <section class="settings-card about-card"><h2>程序信息</h2><Descriptions :column="1" size="small" bordered><DescriptionsItem label="程序名称">{{ draft.programName }}</DescriptionsItem><DescriptionsItem label="程序版本">{{ draft.programVersion }}</DescriptionsItem><DescriptionsItem label="构建版本">{{ draft.buildVersion }}</DescriptionsItem><DescriptionsItem label="发布日期">{{ draft.updateDate }}</DescriptionsItem></Descriptions><div class="company-merged">上海七十迈数字科技有限公司</div><div class="community"><TeamOutlined /><div class="community-content"><strong>{{ draft.communityTitle || '飞书交流群' }}</strong><p>{{ draft.communityText || '使用说明、问题反馈、获取最新版本请扫码加入飞书交流群。' }}</p><AntImage class="community-qr" src="/community-qr.png" :width="148" alt="飞书交流群二维码" :preview="{ mask: '查看大图' }" /><Button v-if="draft.communityUrl" type="link" :href="draft.communityUrl" target="_blank">打开加入链接</Button></div></div></section>
    </div>
  </main>
</template>
