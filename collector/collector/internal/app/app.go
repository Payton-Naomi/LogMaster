...
func New(cfg config.Config, store *spool.Store, logger *slog.Logger, demo bool) (*App, error) {
	...
	client := backend.New(backend.Config{
		...
		Timeout: cfg.Backend.RequestTimeout, Authorization: uploadToken, Gzip: cfg.Backend.UploadGzip,
	})
	worker := backend.NewWorker(backend.WorkerConfig{Interval: cfg.Backend.UploadInterval, MaxFiles: 16, Concurrency: cfg.Backend.UploadConcurrency, InspectBeforeUpload: cfg.Backend.InspectBeforeUpload}, store, client, logger)
...
func (a *App) runPort(ctx context.Context, port config.PortConfig) {
	...
	for ctx.Err() == nil {
		...
		session, err := serialagent.NewSession(serialCfg, serialagent.GoBugFactory{}, func(frameCtx context.Context, frame serialagent.Frame) error {
			...
			for _, line := range decoder.PushFrame(frame.Data, frame.CapturedAt) {
