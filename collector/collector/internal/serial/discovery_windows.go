...
func discoverSystemPorts() ([]PortDescriptor, error) {
	...
	names, namesErr := seriallib.GetPortsList()
	if err != nil && namesErr != nil {
		return nil, errors.Join(err, namesErr)
	}
	locations, locationErr := windowsPortLocations()
	ports := mergePresentPorts(details, names, locations)
...
func mergePresentPorts(details []*enumerator.PortDetails, names []string, present map[string]string) []PortDescriptor {
	byName := make(map[string]PortDescriptor, len(details)+len(names))
	for _, detail := range details {
		port := PortDescriptor{
			...
			Location:     present[strings.ToUpper(detail.Name)],
			Manufacturer: detail.Manufacturer,
			Product:      detail.Product,
			IsUSB:        detail.IsUSB,
		}
		byName[strings.ToUpper(strings.TrimSpace(detail.Name))] = port
	}
	for _, name := range names {
		...
		location, isPresent := present[key]
		if !isPresent {
			continue
		}
		if _, exists := byName[key]; !exists {
			byName[key] = PortDescriptor{Name: strings.TrimSpace(name), Location: location}
		}
	}
	ports := make([]PortDescriptor, 0, len(byName))
	for _, port := range byName {
		ports = append(ports, port)
	}
	sort.Slice(ports, func(i, j int) bool { return strings.ToUpper(ports[i].Name) < strings.ToUpper(ports[j].Name) })
	return ports
}
