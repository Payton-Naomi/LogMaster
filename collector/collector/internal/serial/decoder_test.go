...
func TestDecoderPushFrameFlushesTextWithoutLineEnding(t *testing.T) {
	decoder, _ := NewDecoder(EncodingUTF8)
	at := time.Now()
	lines := decoder.PushFrame([]byte("device output without newline"), at)
	if len(lines) != 1 || lines[0].Text != "device output without newline" {
		t.Fatalf("unexpected frame lines: %#v", lines)
	}
	if decoder.PendingBytes() != 0 {
