...
// PushFrame treats an idle-gap or size-bounded serial frame as a complete
// record boundary, while still honoring CR/LF records inside the frame.
func (d *Decoder) PushFrame(frame []byte, capturedAt time.Time) []DecodedLine {
	lines := d.Push(frame, capturedAt)
	if line, ok := d.Flush(capturedAt); ok {
