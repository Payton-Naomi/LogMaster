...
type BackendConfig struct {
	...
	AuthorizationTokenEnv string        `yaml:"authorization_token_env"`
...
func validateBackend(c BackendConfig) error {
	...
	if c.AuthorizationTokenEnv != "" && !environmentNamePattern.MatchString(c.AuthorizationTokenEnv) {
		return errors.New("backend.authorization_token_env must be an environment variable name")
	}
