...
func TestBackendAuthorizationUsesEnvironmentVariableNameOnly(t *testing.T) {
	yamlText := strings.Replace(minimalYAML, "  base_url: http://localhost:8080/api\n", "  base_url: http://localhost:8080/api\n  authorization_token_env: LOGMASTER_UPLOAD_TOKEN\n", 1)
	cfg, err := LoadBytes([]byte(yamlText))
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Backend.AuthorizationTokenEnv != "LOGMASTER_UPLOAD_TOKEN" {
		t.Fatalf("authorization token env = %q", cfg.Backend.AuthorizationTokenEnv)
	}
	cfg.Backend.AuthorizationTokenEnv = "not valid"
	if err := cfg.Validate(); err == nil || !strings.Contains(err.Error(), "authorization_token_env") {
		t.Fatalf("expected invalid environment name error, got %v", err)
	}
}
